"""Generate conditional denoising in-distribution risk data for Fig. 2c."""
...
def load_history(path_run, device="cuda"):
    """Load the final loss-history window and Bayes baseline from one CDM run."""
...
def main():
    """Evaluate CDM checkpoints over the p-flip grid and write cdm-risk.json."""
    # Loop over p.
    ...
    for p in p_list:
        ...
        # Load the final loss history for each CDM architecture variant.
    ...
    with open(ghm_output_path("cdm-risk.json"), "w") as f:
