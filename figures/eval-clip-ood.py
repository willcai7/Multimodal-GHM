"""Generate CLIP out-of-distribution risk data for Fig. 8a."""
...
def load_model(path_run, device="cuda"):
    """Load a trained CLIP text/image encoder pair from a run folder."""
...
def main():
    """Evaluate CLIP checkpoints on mismatched p-flip samplers and write JSON."""
    # Initialize the mis-specified belief-propagation tree.
    # The checkpoints are trained at p=0.20; the loop below changes the test
    # sampler p to measure out-of-distribution behavior.
    ...
    # Load all CLIP variants once because only the test distribution changes.
    ...
    for p in p_list:
        ...
        # Initialize sampler for the current OOD test distribution.
        ...
        # Compute model losses against the current OOD batch.
    ...
    with open(ghm_output_path("clip-ood.json"), "w") as f:
