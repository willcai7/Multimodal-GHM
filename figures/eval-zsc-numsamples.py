"""Generate zero-shot sample-count scaling data for Fig. 5 and Fig. 6."""
...
if __name__ == "__main__":
    # Use p=0.2 and sweep the zero-shot support size from 1 to 250.
    device = "cpu"
    ...
    data_name = "K4_L4C3p20_L4C3p20sc10"
    ...
    num_samples_list = np.arange(1, 251, 1)
    print(num_samples_list)

    zsc_res =zsc_loss(sampler, model_dicts, num_samples_list, device=device)

    # Fig. 5 and Fig. 6 read this canonical sample-scaling JSON filename.
    with open(ghm_output_path("zsc-numsamples.json"), "w") as f: 
        json.dump(zsc_res, f, indent=4)
