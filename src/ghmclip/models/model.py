...
class ConditionalAutoRegressiveTransformer(nn.Module):
    ...
    def forward(self, xt, zi):
        ...
        if self.auto_regressive:
            self.masks = torch.triu(torch.ones(T1, T1), diagonal=1).bool()
        T = T1 + T2
...
class ConditionalDenoiseEncoderTransformer(nn.Module):
    ...
    def forward(self, xt, zi):
        ...
        for (q, k, v, mlp, ln1, ln2, t_guided_layer_flag, i_guided_layer_flag) in zip(
            ...
            if self.auto_regressive:
                attn_weights = attn_weights + self.masks
            attn_weights = self.activation(attn_weights)
            H = H + torch.einsum('bij,bjd->bid', attn_weights, value) # attention layer + residual connection 
        ...
        for (q, k, v, mlp, ln1, ln2, guided_layer_flag) in zip(
            ...
            if self.guide and guided_layer_flag: # guided layer
                ...
                if guided_counter < self.n_guided_layer: # downward process 
                    ...
                    guide_output = torch.concatenate([h_H, q_H, u_H], dim=2) # concatenate hd, qd and ud
                guided_counter += 1
                guided_layers.append(guide_output)
            
        prediction = self._read_out(H) # read out layer
        if feature:
            return prediction[:,:,0], guided_layers, features
        else:
            return prediction[:,:,0], guided_layers

class EncoderTransformer(nn.Module):
    def __init__(self, n_token, num_class, n_embd=128, n_layer=12, n_guided_layer=3, n_head=4, n_mlp_multiplier=4,
                 activation="softmax",  mlp=True, normalize_attn=True, layernorm=True, maxnorm=False, guide=False, guide_contract=False, auto_regressive=False, rep=True):
        ...
        self.auto_regressive = auto_regressive # whether to use auto-regressive layer
    ...
    def forward(self, x, feature=False):
        if self.auto_regressive:
            self.masks = torch.triu(torch.ones(x.size(1), x.size(1)), diagonal=1).bool()
        B, T = x.size() # batch size and position size
        positions = torch.arange(T, device=x.device).expand(B, T) # position embeddings
        H = self.token_embeddings(x) + self.position_embeddings(positions) # token embeddings + position embeddings 
        guided_layers = []
        ...
        for (q, k, v, mlp, ln1, ln2, guided_layer_flag) in zip(
            ...
            if self.normalize_attn: # normalize attention weights
                attn_weights = attn_weights / np.sqrt(self.n_embd)
            if self.auto_regressive:
                attn_weights = attn_weights.masked_fill(self.masks, float('-inf'))
            attn_weights = self.activation(attn_weights)
            H = H + torch.einsum('bij,bjd->bid', attn_weights, value) # attention layer + residual connection 
        ...
        prediction = self._read_out(H) # read out layer
        if not self.rep:
            prediction = torch.transpose(prediction, 1, 2) # transpose
        ...
        if feature:
            return prediction, guided_layers, features
        else:
            return prediction, guided_layers
...
class GuidedClipLoss(nn.Module):
    def __init__(self, K, batch_size, penalty=1e-4, guide=False):
        ...
        self.guide = guide
        # Pre-compute concat_mat once
        self.concat_mat = torch.kron(torch.ones(self.K-1, 1), torch.eye(self.batch_size))

    def forward(self, tmodel_outputs, imodel_outputs, targets):
        # Move concat_mat to correct device
        concat_mat = self.concat_mat.to(tmodel_outputs[0].device)
        ...
        if self.guide:
            ...
            tguided_diff = torch.stack(tguided_input) - torch.stack(tguided_target)
            iguided_diff = torch.stack(iguided_input) - torch.stack(iguided_target)
            
            loss3 = self.penalty * (
                torch.sum(torch.pow(torch.linalg.norm(tguided_diff, dim=(2,3), ord='fro'), 2),dim=0) +
                torch.sum(torch.pow(torch.linalg.norm(iguided_diff, dim=(2,3), ord='fro'), 2),dim=0)
            )
